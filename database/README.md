...
- En **Vercel** no uses ese host a pelo: el build no llega por IPv6 (`ENETUNREACH`). La app, si detecta Vercel y un host `db.*.supabase.co`, pasa sola al pooler:
  `postgres.tpyygppuszvhpupnvmxp@aws-1-eu-west-1.pooler.supabase.com:6543`
  (usuario `postgres.<id>`, no `postgres`; cluster **aws-1**, no aws-0). `/food` y `/favorites` no se generan en el build.
